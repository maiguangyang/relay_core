...
// 返回: 0 成功, -1 失败
//
extern int LocalShareBridgeCreate(char* roomID);

// LocalShareBridgeStart 启动桥接器并开始监听 UDP
// preferredPort: 首选端口，0 表示自动分配
// 返回: 实际监听端口 (成功), -1 (失败)
//
extern int LocalShareBridgeStart(char* roomID, int preferredPort);

// LocalShareBridgeStop 停止桥接器
// 返回: 0 成功, -1 失败
//
extern int LocalShareBridgeStop(char* roomID);

// LocalShareBridgeDestroy 销毁桥接器
// 返回: 0 成功
//
extern int LocalShareBridgeDestroy(char* roomID);

// LocalShareBridgeGetPort 获取监听端口
// 返回: 端口号 (成功), -1 (未启动)
//
extern int LocalShareBridgeGetPort(char* roomID);

// LocalShareBridgeIsRunning 检查是否正在运行
// 返回: 1 运行中, 0 未运行
//
extern int LocalShareBridgeIsRunning(char* roomID);

// LocalShareBridgeGetStatus 获取桥接器状态
// 返回: JSON 字符串，需要调用 FreeString 释放
//
extern char* LocalShareBridgeGetStatus(char* roomID);
